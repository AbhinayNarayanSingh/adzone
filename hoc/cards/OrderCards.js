...
const OrderCards = ({key, data}) => {
    const { type, plan, Ads, amount, tenure, currency, timeStamp, billingAddress, order_id } = data;
  ...
  return (
    <div key={order_id} className="order-card-outer-container">
        <div className="order-card-container">
            ...
            <div>
                <h2>{type} | {isCustomPlan ? <>{plan} : {Ads} Ads Credit - {tenure} day validity</> : plan}</h2>
                <p className="order_id">ODI : {order_id}</p>
                <p className="order_id">Amount : {currency} {amount}</p>
